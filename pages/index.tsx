...
export default function Home() {
  ...
  const uploadToClient = (event: { target: { files: FileList | null } }) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (!acceptedImageTypes.includes(event.target.files[0].type)) { return }
        ...
        <div className='w-full h-full bg-[url(/yuki.png)] bg-cover'>
          ...
          <div className='flex justify-center'>
            <div className='rounded-xl w-11/12 sm:w-5/6 md:w-1/2 lg:w-1/3 2xl:w-1/4 border-2 border-stone-800 bg-zinc-800/80'>
              <div className='rounded-t-lg relative h-32'>

                <div className='rounded-t-lg w-full h-max top-0 absolute z-1'>
                  <div className='flex justify-center'>
                    <h1 className="text-2xl font-bold mt-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-pink-300">ยินดีตอนรับสู่ EZ Uploader</h1>
                  </div>
                  <br></br>
                  <div className='flex justify-center items-center'>
                    ...
                    <p className='text-xl font-bold text-neutral-200'>&nbsp;&nbsp;ลากไฟล์มาวางตรงนี้</p>
                  </div>
                  <br></br>
                </div>
                <input id="drop_area" type="file" className='rounded-t-lg opacity-0 z-10 w-full h-full' multiple={false} name="myImage" onChange={uploadToClient} />
              </div>
              {createObjectURL != "" ?
                <>
